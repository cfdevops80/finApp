...
const keyOrderMapping = ["title", "quantity", "unit", "formular"]
...
async function fetchAndProcessData() {
  ...
  // template.table6Data = info;
  let curVal = model.get("fixTableRow")
  let tmpArr = info.map(({ type, ...rest }) => rest);
  curVal["table8"] = tmpArr.map(obj => resortKeys(obj, keyOrderMapping));
  console.log(curVal);
